...
package object restclient {
  ...
  object RestClient {
    ...
    trait Service[A] {
      def get(uri: Uri): ZIO[SttpClient with RestClient[A], Throwable, A]
      def post(uri: Uri, a: A): ZIO[SttpClient, Throwable, Response[A]]
      def delete(id: Int): Task[Boolean]
    }
  }

  type RestClient[A] = SttpClient with Has[RestClient.Service[A]]

  def getUser(uri: Uri): RIO[RestClient[User], User] = RIO.accessM(_.get[RestClient[User]].get(uri))
